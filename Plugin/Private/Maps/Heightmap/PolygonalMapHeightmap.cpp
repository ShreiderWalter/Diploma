...
void UPolygonalMapHeightmap::CreateHeightmap(UPolygonMap* PolygonMap, BiomesMgr* BiomeManager, DistributorOfMoistures* MoistureDist, const int32 Size, const FIslandGeneratorDelegate OnComplete)
{
	if (PolygonMap == NULL)
	{
		return;
	...
	MainHeightmapTask::GenerateHeightmapPoints(HeightmapSize, NumberOfPointsToAverage, this, PolygonMap, BiomeManager, generatePoints);
}

void UPolygonalMapHeightmap::CheckMapPointsDone()
{
	HeightmapData = MainHeightmapTask::HeightmapData;
