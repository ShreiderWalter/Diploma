...
FVector2D URiver::GetCenter() const
{
	if (RiverCorners.Num() % 2 == 0)
	{
		...
		FMapEdge edge = GetDownstreamEdge(halfLength - 1);
...
{
	if (FeederRiver == nullptr || FeederRiver == this || FeederRiver == FeedsInto.Key)
	{
		return false;
	}
